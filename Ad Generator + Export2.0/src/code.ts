figma.showUI(__html__, { width: 350, height: 310 }); // UI is visible on load
...
async function cloneAndFillBlank(template: FrameNode,data: any, index: number, ratio: string,num: number) {
  const newAd = (template.clone() as FrameNode);
  newAd.x = index * 1250;
  newAd.y = ratio === "1-1" ? 3000 + (num - 1) * 5500 : num * 5500;
  newAd.name = `${data.lang}_${data.product}_${data.campaign}_${data.hook}_V${data.version}_P${data.photoId}_${ratio === "1-1" ? "11" : "916"}`.toUpperCase();
...
async function generateAllAds(inputData: any) {
  const flag = parseInt(inputData.type)
  gennum += 1
  ...
  const data = {
    ...
    frames: parseInt(inputData.frames) || 1
  }
  if (flag===1) {
    ...
    for (let i = 0; i < data.frames; i++) {
      ...
      for (const [templateName, template] of Object.entries(templates)) {
        const ratio = templateName.includes("1:1") ? "1-1" : "9-16";
        await cloneAndFillBlank(template, filledData, i, ratio,gennum);
      }
    }
  }
